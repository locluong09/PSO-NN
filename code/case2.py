...
def get_best_cases(production, model):
	'''Return the index of best cases by overall mean square error'''
	X = production
	y = model
	mat = y - X.reshape(1,-1)
...
def get_best(production, model):
	#Uus inf norm to find the best cases
	X = production
	y = model
	mat = y - X.reshape(1,-1)
...
def get_best_new(production, model):
	#Compare the maximum value of historical data and simulation cases
	X = production
	y = model
...
def main():
	...
	best_cases_idx = get_best_new(production, y)[:20]
	...
	train_err, val_err = model.fit(X[best_cases_idx], y[best_cases_idx], n_epochs = 100, batch_size=8)
