...
def main():
	...
	best_cases_idx1 = list(get_best_new(production, y)[:20])
	best_cases_idx2 = list(get_best(production, y)[10:20])
	best_cases_idx3 = list(get_best_cases(production, y)[20:30])
	...
	X_train, X_test, y_train, y_test = train_test_split(X[best_cases], y[best_cases], 0.2, shuffle = True, seed = 10)
	X_train, X_val, y_train, y_val = train_test_split(X_train, y_train, 0.2, shuffle = True, seed = 10)
	...
	train_err, val_err = model.fit(X[best_cases], y[best_cases], n_epochs = 500, batch_size=8)
	...
	def func(*args):
		...
		return np.linalg.norm(y_bar - field_log, ord = np.inf)
